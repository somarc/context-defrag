...
const { groupIntoEpisodes } = require('./episodes');
...
// ── Stage timer — CPU + wall time per pipeline phase ────────────────────────
function stageTimer() {
  ...
  return {
      ...
      const usrMs  = Math.round(cpu.user   / 1000);
      const sysMs  = Math.round(cpu.system / 1000);
      return `[PERF] ${label}: ${wallMs}ms wall  cpu ${usrMs}ms usr / ${sysMs}ms sys`;
    }
  };
}
...
async function runPipeline(opts) {
  ...
  // ── Pre-filter: skip Cursor micro-sessions (< 3 turns) ──────────────────
  // Cursor workspaces contain thousands of very short sessions (1–2 turns)
  // that produce only noise in the signal index. They will be handled by
  // episode grouping in a future Phase 1 pass.
  // Bypass with --all-cursor to include all Cursor sessions.
  let sessionsToProcess = dedupedSessions;
  if (!opts.allCursor) {
    const filtered = dedupedSessions.filter(session => {
      if (session.source !== 'cursor') return true;
      if ((session.turnCount || 0) >= 3) return true;
      return false;
    });
    const cursorSkipped = dedupedSessions.length - filtered.length;
    if (cursorSkipped > 0) {
      tui.log(`[FILTER] Skipped ${cursorSkipped} Cursor micro-sessions (< 3 turns) — use --all-cursor to include`);
    }
    sessionsToProcess = filtered;
  }
  ...
  tui.log(`[EPISODE] ${sessionsToProcess.length} sessions → ${episodes.length} episodes`);

  for (const ep of episodes) {
    tui.log(
      `[EPISODE] "${ep.title}" (${ep.workspace || 'unanchored'}) — ` +
      `${ep.sessionCount} session${ep.sessionCount !== 1 ? 's' : ''}, ${ep.source}`
    );
  }

  // Attach episode metadata to each session so the extractor and writers
  // can surface it without being restructured themselves (Phase 1 scope).
  for (const ep of episodes) {
    for (const s of ep.sessions) {
      s._episodeId        = ep.id;
      s._episodeTitle     = ep.title;
      s._episodeWorkspace = ep.workspace;
    }
  }

  // Persist episode rows to DB if --db was supplied
  if (opts.db) {
    try {
      const { initDb } = require('./db/schema');
      const epDb = initDb(opts.db);
      for (const ep of episodes) {
        epDb.exec(
          `INSERT OR REPLACE INTO episodes ` +
          `(title, workspace, started_at, ended_at, thread_ids, metadata, created_at)` +
          ` VALUES (` +
            `'${ep.title.replace(/'/g, "''")}',` +
            `${ep.workspace ? `'${ep.workspace.replace(/'/g, "''")}'` : 'NULL'},` +
            `${ep.started_at.getTime()},` +
            `${ep.ended_at.getTime()},` +
            `'${JSON.stringify(ep.sessions.map(s => s.id))}',` +
            `'{}',` +
            `${Date.now()}` +
          `)`
        );
      }
      tui.log(`[DB] Upserted ${episodes.length} episode${episodes.length !== 1 ? 's' : ''}`);
    } catch (err) {
      tui.log(`[DB] WARN episode upsert failed: ${err.message}`);
    }
  }
  ...
  for (let si = 0; si < total; si++) {
    const session = sessionsToProcess[si];
  ...
  // ── Build signal index (O(mentions), not O(concepts × sessions)) ────────
  const signalIndex = buildSignalIndex(enriched);
...
function parseArgs(argv) {
  const opts = {
    ...
    debug:      false,  // --debug: emit [DEBUG] timing lines for slow renders
  };
